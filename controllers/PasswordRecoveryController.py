...
class PasswordRecoveryController(Controller):
    ...
    def __request(self, req: Request, resp: Response):
        ...
        if not email and not phone_number:
            self.response(resp, HTTPStatus.BAD_REQUEST, error="Se necesita el correo o el número telefónico")
            return
        ...
        user = User.get(value)
        if not user:
            self.response(resp, HTTPStatus.NOT_FOUND, error="Usuario no encontrado")
            return
        ...
        if not user_verification.save():
            self.response(resp, HTTPStatus.INTERNAL_SERVER_ERROR, error="Problema al guardar el otp del usuario")
            return
        ...
        self.response(resp, HTTPStatus.OK, message="OTP enviado satisfactoriamente")
