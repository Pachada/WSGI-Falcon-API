    ...
    def send_email(self, query_limit: int):
        ...
        try:
            with smtplib.SMTP(self.server, self.port) as server:
                ...
                list_of_emails = EmailPool.getAll(
                    and_(
                ...
                for email in emails_to_send:
                    ...
                    self.__save_to_sended(
                        content, email.email, email.template_id, response_code
                    )

                selected = len(emails_to_send)
                send = selected - errors
                print(
                    f'Date and time: {datetime.now().strftime("%d/%b/%Y %H:%M:%S")}, selected: {selected}, sended: {send}, errores: {errors}'
                )
